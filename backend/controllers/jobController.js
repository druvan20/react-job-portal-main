...
export const postJob = catchAsyncError(async (req, res, next) => {
  ...
  if (!salaryFrom || (!salaryTo && !fixedSalary)) {
    //yah to from to provide kre user ya fixedSalary provide kre!
    return next(
      new errorHandler("Please either provide fixed salary or range salary !")
    );
  }
  if (salaryFrom && salaryTo && fixedSalary) {
    return next(
...
export const updateJob = catchAsyncError(async (req, res, next) => {
  ...
  job = await Job.findByIdAndUpdate(id, req.body, {
    //request mai jo bhi data araha hai wo daal do !
    new: true,
    runValidators: true,
...
export const getSinglejob = catchAsyncError(async (req, res, next) => {
  ...
  try {
    ...
    res.status(200).json({
      //response mai status code and job jo find hue hai wo bhej di!
      sucess: true,
      job,
    });
  } catch (error) {
